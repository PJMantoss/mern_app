...
const createPlace = async (req, res, next) => {
    ...
    //Transactions allows you to build multiple operations in isolation. They're build on sessions
     try{
        ...
        user.places.push(createdPlace)// method used by mongoose to establish connection between the 2 models (User & Place). it grabs the createdPlace id and adds it to the place field of the User
    }catch(err){
        const error = new httpError("Creating Place failed, please try again.", 500)
        return next(error); //return error to stop code execution
    }
